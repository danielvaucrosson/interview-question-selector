# Interview Question Selector — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a Streamlit app that lets interviewers filter and select questions from `question-bank.xlsx`, then export the selection as Excel.

**Architecture:** Single-file Streamlit app (`app.py`) with sidebar filters and main area for results/selection. Reads the Excel question bank via pandas on startup. Uses session state for checkbox selections.

**Tech Stack:** streamlit 1.51, pandas 2.3, openpyxl 3.1, xlsxwriter 3.2

---

### Task 1: Create app.py with data loading and sidebar filters

**Files:**
- Create: `app.py`

**Step 1: Create `app.py` with imports, data loading, and sidebar filters**
...
def load_questions() -> pd.DataFrame:
    ...
    # Parse tech column into a list of individual codes
...
# Sub-domain: filtered by selected domains
if sel_domains:
...
Expected: App opens with sidebar showing all filter widgets, no errors.

---

### Task 2: Add filter logic and results table

**Files:**
- Modify: `app.py` (append after sidebar section)

**Step 1: Add filtering logic and results display**
...
for _, row in filtered.iterrows():
    ...
    trunc_q = row["question"][:120] + "..." if len(str(row["question"])) > 120 else row["question"]

    col_cb, col_id, col_q, col_dom, col_tech, col_diff, col_type = st.columns([0.5, 1, 5, 2, 1.5, 1.5, 1.2])
    with col_cb:
        new_val = st.checkbox("sel", value=checked, key=f"cb_{qid}", label_visibility="collapsed")
...
**Step 2: Run and verify filtering works**

Run: `streamlit run app.py`
Expected: Selecting domains/techs narrows results. Checkboxes toggle selection. Buttons work.

---

### Task 3: Add selected questions panel and Excel export

**Files:**
- Modify: `app.py` (append after results section)

**Step 1: Add selected questions review and export**
...
if selected_df.empty:
    st.info("No questions selected yet. Use the checkboxes above to build your interview set.")
else:
    ...
    with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
        ...
        ws.set_column("B:B", 60)  # question
        ws.set_column("C:C", 80)  # answer
        ws.set_column("D:D", 20)  # domain
    buf.seek(0)
...
**Step 2: Run full app end-to-end**

Run: `streamlit run app.py`
Expected: Filter → select questions → see them expanded below → download Excel with all data.

---

### Task 4: Polish and final verification

**Step 1: Verify column headers display above the results rows**

Add a header row above the checkbox results loop (before the `for _, row in filtered.iterrows():` line):

```python
# Header row
hdr_cb, hdr_id, hdr_q, hdr_dom, hdr_tech, hdr_diff, hdr_type = st.columns([0.5, 1, 5, 2, 1.5, 1.5, 1.2])
with hdr_id:
    st.markdown("**ID**")
with hdr_q:
...
Run: `streamlit run app.py`
Expected: Clean layout, all filters work, selection persists, export downloads correctly.
